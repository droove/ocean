function [SHF LHF Tskin ]=ESROBHF_cor30a(time,zSpd,zWD,zTemp,zBP,zRH,psp,pir,Latitude,Longitude,Temp,Pres,u,v)
% calculation of turbulent heat flux using coare 3.0 algorhythm with ESROB
% data
% output 
% SHF: sensible downward positive heat flux
% LHF: Latent downward positive heat flux
%
% written by JHPark 2016 Dec 21
% 
...
%%%%%%%%%%%%%%%%% data %%%%%%%%%%%%%%%%%%
% load '../data/ESROB_Leg17_fin_L2.mat'
% 
% fname_out='ESROB_Leg17_TurbHeatFlux'


zu=3.4;%wind speed measurement height (m)
zt=3;%air T measurement height (m)
zq=2.4;%air q measurement height (m)
ts_depth=nanmean(Pres);%bulk water temperature sensor depth, U/APL MSP&

% date
for ii=1:length(time)
jds(ii)=str2num(sprintf('%4d%02d%02d%02d%02d%02d\n', time(ii,:)));
end

% Q estimation
hPa2Pa=100;
outsh = convert_humidity (zBP*hPa2Pa, zTemp+273.15, zRH,'relative humidity',...
    'specific humidity', 'Murphy&Koop2005',0.0001); % specific humidity in kg/kg
aspecHM=outsh*1000;% Bulk air specific humidity in g/kg

% us estimation
[wu,wv]=wind2cur(zSpd,zWD);
cm2m=1/100;
current=[u(:,1)*cm2m v(:,1)*cm2m];
wind_norm=[wu./zSpd,wv./zSpd]; 

jdy=jds;%time in the form YYYYMMDDHHSS.SS
U=zSpd; %true wind speed, m/s; etl sonic anemometer
tsnk=0*ones(size(U));%sea snake temperature, C (0.05 m depth)
ta=zTemp;%air temperature, C (z=14.5 m)
qa=aspecHM;%air specific humidity, g/kg (z=14.5  m)
rs=psp;%downward solar flux, W/m^2 (ETL units)
rl=pir;%downward IR flux, W/m^2 (ETL units)
org=0*ones(size(U));%rainrate, mm/hr (ETL STI optical rain gauge, uncorrected)
lat=nanmean(Latitude)*ones(size(U));%latitude, deg  (SCS pcode)
lon=nanmean(Longitude)*ones(size(U));%longitude, deg (SCS pcode)
msp=Temp;%6-m deotg T from MSP, C    
Ps=zBP; %Atmos surface pressure (mb = hPa )
...
valhdr={'jdy', 'U', 'tsnk', 'ta', 'qa', 'rs', 'rl', 'org', 'lat', 'lon', 'msp'}
for ii=1:11
eval(['x(:,ii)=' valhdr{ii} ';'])
end
...
for ibg = 1:nx 			%major read loop
	...
	P=Ps(ibg);                     %air pressure
	us=current(ibg,:)*wind_norm(ibg,:)';  %surface current
    ts=msp(ibg);
	zi=600;                    %inversion ht
			%*******   decode date  ************
   ...
   out(ibg,3)=hsb; %sensible heat flux (w/m^2)
   out(ibg,4)=hlb; %latent heat flux (w/m^2)
   out(ibg,5)=ts-dter*jcool;%cool skin delta
   out(ibg,6)=taub;  %stress
   out(ibg,7)=wbar; %webb mean w (m/s)
   out(ibg,8)=RF; %rain heat flux(w/m^2)
   out(ibg,9)=dter; %cool skin temperature depression (K)
   out(ibg,10)=dt_wrm;
   out(ibg,11)=tk_pwp;
   out(ibg,12)=tkt*1e3;
   out(ibg,13)=Wg; 
...
LHF=-out(:,4);
Tskin=out(:,5);
readme='Downward heat flux (W m^-2); makefile:D:\park\study\ocean\heat_flux\example\code\Cal_heatflux_example.m';
% prtit=1;

% save( fname_out, 'SHF','LHF', 'timevec','readme')


% x=[zSpd us Temp zTemp spcHM psp pir Qs Q psp pir rain zi zBP zu lat jcool jwave w_SP w_SH]
