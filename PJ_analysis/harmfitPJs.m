function [xout harmcon]=harmfitPJs(xin,t,s,nlin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fband =[.00010 .00417;
    ...
    .23320 .25100;
    .26000 .29000;
    .30000 .50000]*24; %cycle per day

% fband=[0.00240000000000000,0.100080000000000];
% for ii=1:length(fu)
% fband=[fband;fu(ii)-.2 fu(ii)-.2];
% end
% fband=[fband ; 7.20000000000000,12];
% fband=[0 12]
...
% 
% [Pxr,fx]=psd(real(xxx),nx,1/dt); % Call to SIGNAL PROCESSING TOOLBOX - see note in t_readme. If you have an error here you are probably missing this toolbox
% warning('off')
% [Pxi,fx]=psd(imag(xxx),nx,1/dt); % Call to SIGNAL PROCESSING TOOLBOX - see note in t_readme.
% warning('off')
% [Pxc,fx]=csd(real(xxx),imag(xxx),nx,1/dt); % Call to SIGNAL PROCESSING TOOLBOX - see note in t_readme.
% warning('off')
